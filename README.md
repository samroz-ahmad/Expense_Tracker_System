...
I declare that I have read and completed the above checked item in the list: Samroz Ahmad Shoaib, 6 Oct 2024.
...
- *Concept #1*: Use of unordered_map for fast lookups when managing users, *File name / Line numbers(s)*: admin.h (lines 12, 15), admin.cpp (line 6)
- *Concept #2*: Use of file I/O streams to append data without overwriting it, which optimizes report generation by allowing incremental updates without recreating entire files, *File name / Line numbers(s)*: expense_manager.cpp (line 18)
- *Concept #3*: Use of exception handling to manage user input validation and file I/O errors. the input validation in add_expense() catches exceptions and retries, reducing runtime errors, *File name / Line numbers(s)*: main_menu.cpp (lines 138-145)
- *Concept #4*: Lazy loading of user data from files in the UserManager class to optimize memory usage by only loading user data when necessary, *File name / Line numbers(s)*: user_manager.cpp (lines 14-17)
...
- *Concept #1*: Encapsulation, *File name / Line numbers(s)*: user.h (lines 9-19)
- *Concept #2*: Polymorphism, *File name / Line numbers(s)*: user.h (lines 23-31)
- *Concept #3*: Abstraction, *File name / Line numbers(s)*: expense.h (line 18), expense.cpp (line 9)
- *Concept #4*: Constructor and Destructor, *File name / Line numbers(s)*: expense_manager.cpp (line 53)
